...
export function getTypes(data: PullRequest) {
...
export function getTypeAndScore(data: PullRequest) {
  return `labels: ${getTypes(data).join(',')}, score: ${getScore(data)}`;
}
...
class PullRequestReviewHandlers {
  submitted(event: PullRequestReviewSubmittedEvent) {
    ...
    if (cond1 || cond2) {
      const title = `${getRepoName(repository)}: [#${pull_request.number} ${
        pull_request.title
